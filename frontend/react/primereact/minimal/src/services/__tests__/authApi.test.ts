...
describe('Auth API', () => {
  ...
  describe('login', () => {
    it('posts credentials to /auth/login', async () => {
      const mockResponse = { data: { success: true, data: { accessToken: 'tok', refreshToken: 'rt', expiresIn: 3600, user: { id: 1 } } } }
  ...
  describe('getProfile', () => {
    it('gets from /auth/profile', async () => {
      const mockResponse = { success: true, message: '', data: { id: 1, username: 'admin' } }
  ...
  describe('updateProfile', () => {
    it('puts to /auth/profile', async () => {
      ...
      const mockResponse = { success: true, message: '', data: { id: 1, email: 'updated@test.com' } }
  ...
  describe('refreshToken', () => {
    it('posts to /auth/refresh with token', async () => {
      const mockResponse = { data: { success: true, data: { accessToken: 'new-tok', refreshToken: 'new-rt', expiresIn: 3600 } } }
      vi.mocked(api.post).mockResolvedValue(mockResponse)
  ...
  describe('resetPassword', () => {
    it('posts to /auth/reset-password with positional args', async () => {
      vi.mocked(api.post).mockResolvedValue({})
  ...
  describe('changePassword', () => {
    it('posts to /auth/change-password with positional args', async () => {
      vi.mocked(api.post).mockResolvedValue({})
